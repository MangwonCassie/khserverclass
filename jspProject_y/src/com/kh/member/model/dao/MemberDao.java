...
public class MemberDao {
	...
	//dao에 접근하는 xml파일이 다 다르다. 
	public MemberDao() {
		
		//Class.getResource() : 클래스 기준으로 상대적인 위치의 리소스를 가져올 수 있다.
		String filePath = MemberDao.class.getResource("/sql/member/member-mapper.xml").getPath();
		
		try {
			//FileInputStream(String name) 주어진 이름이 가리키는 파일을 바이트 스트림으로 읽기 위한 FileInputStream 객체생성
	...
	public int checkId(Connection conn, String checkId) {
		...
		//select 문 조회지만 결과값은 count 하나만 받아올 것
		
		int count = 0;
		ResultSet rset = null; //어찌됐건 조회는 해와야하니까
		...
		//자체 null.........키값으로 못불러옴 sql = prop.getProperty(sql); 저 매개변수로는 당연히 null이니까..못 불러오지.....
