...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(loginUser ==null) { //로그인 실패 (에러 페이지로 응답해보기) 
			...
			//기존에 보고 있던 화면  로그인하고 나서도 그대로 있게 처리하는 거임
			//Referrer: 이라고 방금전에 있었던 경로를 "속성"으로 가지고 있다.    - > 이전 경로 찾을 수 있음 
			
			 String before = request.getHeader("Referer");
			 //System.out.println(before);
			 //System.out.println(before); 찍으면 http://localhost:8888/jsp/ 해당 정보를 redirect로 보내주면 된다고 함.
			 //"Referrer" 속성이 웹페이지에 있기 때문에 뭘 할 필요 x
			
			response.sendRedirect(before);
			//로그아웃 - > 10페이지 보고 있다가 로그인해도 10페이지에 있음 (사용자 편의성 증진) - > 꼭 응용하기
			
			
			//response.sendRedirect(request.getContextPath()); 이거는 로그인 안한 상태로 게시판 보다가 아이디로그인하면 날라감
