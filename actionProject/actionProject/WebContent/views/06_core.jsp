...
<body>
	...
	<pre>
		*변수 선언(&lt;c:set var="변수명" value="리터럴" scope="스코프영역지정(생략가능)"&gt;)
		...
		-예를 들어, 정수형 변수 "num"에 10을 할당하려면 다음과 같이 작성할 수 있습니다.
		...
		-c:set을 통해 선언된 변수는 EL표기법을 통해 접근가능하다 (스크립팅원소로는 접근 불가)
	...
	<!--Unknown tag (c:set) - 선언해야한다. taglib  -->
	...
	<!-- 변수명만 제시한다면 작은 범위의 스코프 영역부터 찾아지게 된다.불필요한 조회가 이루어질수있으니 해당 영역을 지정하는것을
		   권장한다. -->
	${pageScope.num1} <br>
	${requestScope.num2} <br>
	${sessionScope.num3} <br>
	${requestScope.num3} <br>
	...
	<pre>
		*변수 삭제 (removeAttribute) 
		(&lt;c:remove var="제거하고자하는변수명" scope="스코프영역지정(생략가능)"&gt;)
...
<%--
	...
	<pre>
		for loop문 - (&lt;c:forEach var="변수명" 
						begin="초기값" end="끝값" step="증가시킬값(생략가능)"&gt;)
	...
	<pre>
		&lt;c:forTokens var="각값을 담을변수" items="분리시키고자하는 문자열" delims="구분자"&gt;
	...
	<ul>
		...
		<!-- 이 세 개의 구분자 중 하나가 문자열에 포함되면 토큰으로 분리되어 리스트로 출력 -->
			<li>${d}</li>
		</c:forTokens>
	</ul>
