...
<body>
	...
	<p>
		...
		내부적으로 해당 scope 영역에 키값에 해당하는 벨류값을 꺼내올 수 있음. <br>
		기본적으로 EL은 JSP 내장 객체를 구분하지 않고 자동적으로 모든 내장객체에 키값을 검색하여
		존재하는 경우 해당 값을 가져온다. 
	</p>
	...
	<p>
		...
		<%--
			...
			(때문에 getter메소드는 필수로 작성하여야한다)
			 ...
			 JSP에서 EL(Expression Language)을 사용하여 ${lecture.name}과 같이 속성을 참조할 때, 해당 객체의 getName() 메소드와 같은 getter 메소드를 호출
		 --%>	
	...
	<h3>3.EL사용시 "내장객체"들에 저장된 키값이 동일할 경우</h3>
	scope 값 : ${ scope } <br>
	
	<!-- 
		EL은 공유범위가 "가장 작은 scope에서부터 해당 키값"을 검색한다.
		page->request->session->application 순으로 속성을 찾는다 (P R S A)
	...
	잘못된 접근 예시 : session에 classRoom 접근 해보기 - ${sessionScope.classRoom }<br> 
