...
public class ELServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 값이 한번 담기면 서버가 멈추거나 브라우저가 닫힐때까지 사용 가능하다 
		 * 
		 * 3.HttpServletRequest (request scope)
		 * 요청 및 응답시 매번 생성되는 객체
		 * 이 영역에 데이터를 담으면 "해당 request객체를 포워딩 받는 응답 페이지"에서 사용 가능하다 
		 * 
		 * 4.PageContext (page scope)
		 * 현재 jsp 페이지에서만 사용 가능 (페이지를 벗어나면 소멸)
		 * -공유 범위가"가장 작음"
		 * 
		 * 위 객체들 모두 값을 담을땐 .setAttribute("키",데이터);
		 * 		 	값을 꺼낼땐 .getAttribute("키");
		 * 			값을 지울땐 .removeAtrribute("키");
		 * 
		 * */
		
		//"classRoom"과 "student"라는 두 개의 속성(attribute)을 설정함

		//"classRoom" 속성의 값으로는 "E강의장" 문자열(String)을 전달
		//"student" 속성의 값으로는 Person 클래스의 인스턴스인 new Person("김길동","남자")를 전달
		...
		//request영역과 session 영역에 같은 키값으로 데이터 담아보기
